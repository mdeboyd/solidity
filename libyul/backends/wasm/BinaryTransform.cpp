...
bytes BinaryTransform::operator()(Literal const& _literal)
{
	if (holds_alternative<uint32_t>(_literal.value))
		return toBytes(Opcode::I32Const) + lebEncodeSigned(static_cast<int32_t>(get<uint32_t>(_literal.value)));
	else if (holds_alternative<uint64_t>(_literal.value))
		return toBytes(Opcode::I64Const) + lebEncodeSigned(static_cast<int64_t>(get<uint64_t>(_literal.value)));
	else
